...
public class TetheredAStarPathfinding {
	...
	public static boolean getShortestPath(
		...
		AStarSubPath initialSubPath = new AStarSubPath(initialPredecessors, initialTetherConfiguration);
			...
			else
			{
				...
				for (AStarEdge edge : current.edges) {
					...
					if (current.g + edge.cost <= neighbour.g) {
						// For each possible sub-path, compute the changes in the tether configurations.
						for (int i = 0; i < current.subPaths.size(); i++) {
							AStarSubPath subPath = current.subPaths.get(i);

							TetherConfiguration newTC = computeTetherChange(subPath.tc, maxTetherLength, neighbour.p, g.obstacles);

							// Create a new shortest path if the tether configuration is valid.
							// Note that tether length checks have already been made.
							if (newTC != null) {
								List<AStarNode> newPredecessorList = new ArrayList<AStarNode>();
								newPredecessorList.addAll(subPath.predecessorList);
								newPredecessorList.add(current);
								
								AStarSubPath newSubPath = new AStarSubPath(newPredecessorList, newTC);
	...
	/**
	 * Retrieves any path to the destination.
	 * Note that we do not prioritise any paths.
	 * We could for example choose the path with the shortest final tether configuration.
	 * 
	 * @param destination
	 * @return path
	 */
	public static Path retrievePath(AStarNode destination) {
		...
		if (destination.subPaths.size() == 0) {
			Log.error("No sub-path exists.");
			
			return null;
		}
		
		// Get an arbitrary predecessor list - in this case the first.
		List<AStarNode> predecessorList = destination.subPaths.get(0).predecessorList;
		
		// Reassemble the forward path.
		for (int i = predecessorList.size() - 1; i >= 0; i--) {
			path.addPoint(predecessorList.get(i).p);
		}
	...
	private static TetherConfiguration computeTetherChange(
		...
		// Iteratively move towards the destination, altering the tether configuration appropriately on each iteration.
		while (!currentPoint.equals(destination)) {
			// Get the next tether configuration by moving towards the destination.
			TetherConfiguration nextTC = getNextTetherConfiguration(currentTC, destination, obstacles);

			// Fail if the tether configuration did not change.
			// Note that it is fine if the robot did not move, so long as wrapping or unwrapping of the tether occurred.
			...
			currentTC = nextTC;
			currentPoint = ListFunctions.getLast(currentTC.points);
		}
	...
	private static TetherConfiguration getNextTetherConfiguration(
		...
		/*
		 * If s equals 2 then we cannot unwrap since there is no tether (segment rq) to unwrap, and specifically no point r.
		 * As such we will set the appropriate variables to null, namely r, rq and u.
		 * Note that the code is structured such that if r is undefined then unwrapping will not be checked.
		 */
		...
		if (minVertex != null) {
			...
			// Remove the last point on the tether (p), and add new vertices, minVertex and newP.
		...
		if (u != null) {
			// Remove the last two points on the tether (p and pivot point q), and add new vertex u.
			newTC.removeLastPoint();
			newTC.removeLastPoint();
			newTC.addPoint(u);
			return newTC;
